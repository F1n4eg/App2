...
//class PatternSearch { // критерій хюіжності
//public:
//    PatternSearch(vector<double> startPoint, double stepSize, double tolerance, int maxIterations)
//        : x(startPoint), stepSize(stepSize), tolerance(tolerance), maxIterations(maxIterations) {
//    }
//
//    void search() {
//        int n = x.size();
//        vector<double> direction(n, 0.0);
//        int iter = 0;
//
//        auto start = chrono::high_resolution_clock::now();
//
//        while (stepSize > tolerance && iter < maxIterations) {
//            vector<double> bestX = x;
//            double bestVal = f(x[0], x[1], x[2], x[3]);
//
//            for (int i = 0; i < n; ++i) {
//                vector<double> tempX = x;
//                tempX[i] += stepSize;
//
//                if (checkConstraints(tempX[0], tempX[1], tempX[2], tempX[3])) {
//                    double newVal = f(tempX[0], tempX[1], tempX[2], tempX[3]);
//                    if (newVal < bestVal) {
//                        bestVal = newVal;
//                        bestX = tempX;
...
//            }
//
//            if (bestVal < f(x[0], x[1], x[2], x[3])) {
//                x = bestX;
//                //cout << "New Best Fitness: " << bestVal << "\n";
//            }
//            else {
//                stepSize /= 2.0;
//            }
...
//
//        cout << "Execution Time: " << elapsed.count() << " seconds\n";
//        cout << "Best Fitness: " << f(x[0], x[1], x[2], x[3]) << "\n";
//        cout << "Best Solution: (" << x[0] << ", " << x[1] << ", " << x[2] << ", " << x[3] << ")\n";
//    }
//
//private:
//    vector<double> x;
